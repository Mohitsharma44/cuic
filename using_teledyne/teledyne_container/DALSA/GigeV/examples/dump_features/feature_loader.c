		...
		if (camIndex != -1)
		{
			...
			if (status == 0)
			{
				...
				if (status == GEVLIB_OK)
				{
					...
					status = GevSetFeatureValueAsString( handle, "Std::DeviceRegistersStreamingStart", " ");
					if ( status == 0 )
					{					
						// Verify that we are in "streaming feature mode"/
						{
							...
							{
								Sleep(10);
								status = GevGetFeatureValue( handle, "Std::DeviceRegistersStreamingStart", &type, sizeof(done), &done);
								if (status != 0)
								{
									done = TRUE;
								}
						...
						{
							printf("Error - unable to initialize feature streaming! (DeviceRegistersStreamingStart command failed)\n"); 
						}

						// End the "streaming feature mode".
						status = GevSetFeatureValueAsString( handle, "Std::DeviceRegistersStreamingEnd", " ");
						if ( status == 0 )
						{					
							// Verify that we are in "streaming feature mode"/
							{
								...
								{
									Sleep(10);
									status = GevGetFeatureValue( handle, "Std::DeviceRegistersStreamingEnd", &type, sizeof(done), &done);
									if (status != 0)
									{
										done = TRUE;
									}
