...
try:
    ...
    birger = api.Birger(os.getenv("birger_uds_host"),
                        os.getenv("birger_uds_port"))
except Exception as ex:
    logger.critical("Cannot import some modules: "+str(ex))
"""
...
def on_rpc_request(ch, method, props, body):
    ...
    try:
        ...
        response = "ERR"
        logger.info("Message: "+str(command_dict))
        #logger.info("commands received: "+str(command_dict))
...
if __name__ == "__main__":
    ...
    flags = mngr.dict({'capture': 0, 'ready_flag': False, 'resend_flag': False})
    ...
    ptc = PTCcontroller(flags, status_dict, interval, command, noimage, loop_cntr)
