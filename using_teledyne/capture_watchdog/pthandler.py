...
class PTCcontroller(object):
    ...
    def ptcontrol(self):
        ...
        def _new_file_captured():
            ...
            while trial < 2:
                new_ftime = self._get_last_file_modified_time(os.path.join(os.getenv("mtc_vis_dir"),
                                                                            ...
                                                                            "live"))
                # If new files were not captured and if we dont need to resend the previous command
                if not (new_ftime > self._old_ftime) and not self.flags["resend_flag"]:
                    # maybe camera is slow? network issue? Give it 5 more seconds:
                    trial += 1
                else:
                    return new_ftime
                time.sleep(5)
        ...
        try:
            ...
            if (self.loop_cntr.value > 0 or self.loop_cntr.value == -1):
            try:
            logger.info("Inside self.loop_cntr condition.. so either its > 0 or = -1")
            if not self.noimage.value:
            logger.info("Checking _new_ftime")
            _new_ftime = self._get_last_file_modified_time(os.path.join(os.getenv("mtc_vis_dir"),
            self.command["location"],
            "live"))
            # If new files were not captured,
            if not (_new_ftime > self._old_ftime):
            # restart the camera!
            logger.info("no new files were captured, restart the camera")
            self.restart_camera()
            while not self.flags["ready_flag"]:
            ...
            logger.info("Stop iteration raised")
            self.pt_positions = iter(self.command["pt_locs"])
            if self.loop_cntr.value > 0:
            logger.info("decrementing self.loop_cntr")
            self.loop_cntr.set(self.loop_cntr.value - 1)
            logger.info("decremented loop cntr = {}".format(self.loop_cntr.value))
            """
